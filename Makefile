...
LAST_COMMIT := $(or $(shell git rev-parse --short HEAD),$(shell head -n 1 VERSION | grep -oP -m 1 "^[a-z0-9]+$$"),"UNKNOWN")

# Try to get the semver from 1) git 2) the VERSION file 3) fallbakc.
VERSION := $(or $(shell git describe --tags --abbrev=0),$(shell grep -oP "tag: \K(.*)" VERSION),"v0.0.0")
